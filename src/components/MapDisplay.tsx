...
interface MapDisplayProps {
  ...
  onSpotRemove?: ((spotId: string) => void) | ((teamId: number, type: 'main' | 'alt') => void)
  spotTypes?: {
    type1: SpotType
    type2: SpotType
    type3: SpotType
            ...
            >
              ...
              {onSpotRemove && allowSpotAdd && (
                <button
                  onClick={e => { 
                    e.stopPropagation(); 
                    if (spot.teamId !== undefined && spot.type && typeof spot.type === 'string' && (spot.type === 'main' || spot.type === 'alt')) {
                      // For lootspot page
                      (onSpotRemove as (teamId: number, type: 'main' | 'alt') => void)(spot.teamId, spot.type);
                    } else if (spot.id) {
                      // For dali page
                      (onSpotRemove as (spotId: string) => void)(spot.id);
                    }
                  }}
