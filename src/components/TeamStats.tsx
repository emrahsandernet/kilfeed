...
export default function TeamStats({ teams, onReviveAllTeams, onResetAllPositions, onLoadTeamList }: TeamStatsProps) {
  ...
  const totalPlayers = teams.reduce((sum, team) => sum + (team.players?.filter(alive => alive === 'true')?.length || 0), 0)
  const totalTeams = teams.length
  const squadCount = teams.filter(team => (team.players?.filter(alive => alive === 'true')?.length || 0) === 4).length
  const threeManCount = teams.filter(team => (team.players?.filter(alive => alive === 'true')?.length || 0) === 3).length
  const twoManCount = teams.filter(team => (team.players?.filter(alive => alive === 'true')?.length || 0) === 2).length
  const soloCount = teams.filter(team => (team.players?.filter(alive => alive === 'true')?.length || 0) === 1).length
  const eliminatedTeams = teams.filter(team => (team.players?.filter(alive => alive === 'true')?.length || 0) === 0).length
  ...
  return (
    <div className="mb-4">
      ...
      <div className="grid grid-cols-6 gap-2">
        ...
        <div className="bg-gray-800 rounded-lg p-1 text-center">
          <div className="text-xs text-gray-400">Eliminated Teams</div>
          <div className="text-lg font-bold text-gray-500">{eliminatedTeams}</div>
        </div>
      </div>
    </div>
  )
